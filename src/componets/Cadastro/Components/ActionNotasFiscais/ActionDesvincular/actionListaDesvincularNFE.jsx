...
export const ActionListaDesvincularNFE = ({
  ...
  const dados = dadosPedidosVinculados?.data
    .filter((item) => {
      const idNota = item.NOTAVINC;
      return (!idNota || idNota !== dadosPedidosVinculados.idNotaFiscal);
    }).map((item, index) => {
  ...
  useEffect(() => {
    ...
    return (!item.STATUSVINC || item.STATUSVINC === 'False') && 
            (!item.NOTAVINC || item.NOTAVINC !== dadosPedidosVinculados.idNotaFiscal) &&
    ...
    if (checked) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          setSelectedItems(itensSelecionaveis.map(item => ({ ...item, idNotaFiscal: dadosPedidosVinculados.idNotaFiscal })));
          setSelectedIds(itensSelecionaveis.map(item => item.IDPEDIDO));
          setPedidosSelecionados(itensSelecionaveis.map(item => ({ ...item, quantidade: 1, idNotaFiscal: dadosPedidosVinculados.idNotaFiscal })));
          setSelectAll(true);

        } else if (result.dismiss === Swal.DismissReason.cancel) {
          const itensSelecionaveisPaginaAtual = dados.slice(first, first + rows).filter(item => item.stDisabled !== 'disabled');

          setSelectedItems(itensSelecionaveisPaginaAtual.map(item => ({ ...item, idNotaFiscal: dadosPedidosVinculados.idNotaFiscal })));
          setSelectedIds(itensSelecionaveisPaginaAtual.map(item => item.IDPEDIDO));
          setPedidosSelecionados(itensSelecionaveisPaginaAtual.map(item => ({ ...item, quantidade: 1, idNotaFiscal: dadosPedidosVinculados.idNotaFiscal })));
          setSelectAll(true);
            ...
            <input
              ...
              onChange={(e) => {
                ...
                const updatedSelectedItems = checked
                  ? [...selectedItems, { ...row, idNotaFiscal: dadosPedidosVinculados.idNotaFiscal }]
                  : selectedItems.filter(item => item.IDPEDIDO !== row.IDPEDIDO);
                setSelectedItems(updatedSelectedItems);

                const updatedPedidosSelecionados = checked
                  ? [...pedidosSelecionados, { ...row, quantidade: 1, idNotaFiscal: dadosPedidosVinculados.idNotaFiscal }]
    ...
    <Fragment>
      <div className="panel">
        <div className="panel-hdr">
          <h2>Lista de Pedidos Vínculados</h2>
        </div>
