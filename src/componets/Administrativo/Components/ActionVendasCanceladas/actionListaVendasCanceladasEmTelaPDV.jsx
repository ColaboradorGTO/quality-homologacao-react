...
export const ActionListaVendasCanceladasEmTelaPDV = ({ 
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: 'Vendas Canceladas Tela PDV',
  });
  ...
  const exportToPDF = () => {
    ...
    doc.save('vendas_canceladas_tela_pdv.pdf');
  };

  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(dadosListaVendasCanceladas);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Vendas Canceladas Tela PDV');
    XLSX.writeFile(workbook, 'vendas_canceladas_tela_pdv.xlsx');
  };
  ...
  const calcularTotalValorDinheiro = () => {
    let total = 0;
    for (let dados of dadosVendasCanceladasEmTelaPDV) {
  ...
  const dadosListaVendasCanceladas = dadosVendasCanceladasEmTelaPDV.map((item, index) => {
  ...
  const colunasVendasCanceladas = [
    ...
    },
    {
      ...
      header: 'Motivo',
      body: row => <th style={{ textTransform: 'uppercase' }}> {row.TXTMOTIVOCANCELAMENTO}</th>,
      sortable: true,
    ...
    <Fragment>
      <div className="panel">
        <div className="panel-hdr">
          <h2>Vendas Canceladas Tela PDV</h2>
        </div>
