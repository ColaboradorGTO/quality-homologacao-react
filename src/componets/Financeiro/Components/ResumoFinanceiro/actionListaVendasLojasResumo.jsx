...
export const ActionListaVendasLojasResumo = ({ dadosTotalVendasEmpresa, dataPesquisa }) => {
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.autoTable({
      head: [['Data', 'Loja', 'Valor Bruto', 'Dinheiro', 'Cartão', 'POS', 'PIX', 'Convênio', 'Fatura Pix', 'Fatura', 'Despesa', 'Disponível']],
  ...
  const calcularValorTotalPix = () => {
    return calcularTotal('VALORTOTALPIX');
  }

  const calcularValorTotalConvenio = () => {
    return calcularTotal('VALORTOTALCONVENIO');
  }
  const calcularValorTotalFaturaPix = () => {
    return calcularTotal('VALORTOTALFATURAPIX');
  }
