...
export const ActionPesquisaDescontoVendas = () => {
  ...
  const fetchDescontoMotivoVendas = async () => {
    ...
    const urlBase = `/desconto-motivo-vendas?idEmpresa=${empresaSelecionada}&idMarca=${marcaSelecionada}&dataPesquisaInicio=${dataPesquisaInicio}&dataPesquisaFim=${dataPesquisaFim}`;
    let urlApi = urlBase.includes('?') ? urlBase : urlBase + '?';
    urlApi = urlApi.replace('&page=1', '').replace('page=1', '');
    try {
      const primeiraPagina = 1;
