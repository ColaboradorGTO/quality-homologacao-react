...
export const ActionListaVendaSefaz = ({ dadosSefaz, usuarioLogado, optionsModulos }) => {
  const [globalFilterValue, setGlobalFilterValue] = useState('');
  const [expandedRows, setExpandedRows] = useState([]);
  const [rowSelection, setRowSelection] = useState(null);
  const dataTableRef = useRef();
  const { onSubmit } = useConsultarNFCe({ dadosSefaz, usuarioLogado, optionsModulos });
  const { onSubmitNFe } = useConsultarNFe({ dadosSefaz, usuarioLogado, optionsModulos });
  ...
  const dados = dadosSefaz?.vendaData.data?.map((item, index) => {
    let contador = index + 1;
   
    return {
      contador,
      IDVENDA: item?.venda.IDVENDA,
      NOFANTASIA: item?.venda.NFE_INFNFE_EMIT_FANT,
      DTHORAFECHAMENTO: item?.venda.DTHORAFECHAMENTO,
      DSCAIXA: 'Caixa Web',
      NOFUNCIONARIO: 'Myltiane',
      ...
      NRNOTA: item?.venda.NFE_INFNFE_IDE_NNF,
      UF: item?.venda.NFE_INFNFE_EMIT_ENDEREMIT_UF,
      CHAVE: item?.venda?.CHAVE,
      CSTAT: item?.venda?.PROTNFE_INFPROT_CSTAT,
      ...
      VALORRECEBIDO: toFloat(item.pagamento[0].pag.VALORRECEBIDO),
      NPARCELAS: item.pagamento[0].pag.NPARCELAS,
      NOAUTORIZADOR: item.pagamento[0].pag.NOAUTORIZADOR,
  ...
  const ExpandedCompenent = ({ data }) => {
    console.log(data, 'DATA EXPANDDED')
    return (
      <>
        <div className="mt-2">
          <table
      ...
      <div className="panel">
        ...
        <div className="card" ref={dataTableRef}>
          ...
          >
            {coluna.map(coluna => (
              <Column
                key={coluna.field || 'expander'}
                expander={coluna.expander}
                field={coluna.field}
